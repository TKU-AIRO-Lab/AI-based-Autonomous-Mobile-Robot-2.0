...
class LineFollowerBase:
    ...
    def process_image(self, cv_image, turn_direction="straight"):
        ...
        if turn_direction == "left": c_ratio = 0.2
        elif turn_direction == "right": c_ratio = 0.8
        else: c_ratio = 0.5
        ...
        # 完整掃描後選信心度最高的偵測結果（雙線優先，其次單線）
        best_dual_x, best_dual_dist   = None, float('inf')
        best_single_x, best_single_dist = None, float('inf')
            ...
            if turn_direction == "double_yellow":
                if len(y_coords) > 0:
                    ...
                    if right_y - left_y > w * 0.2:
                        ...
                        both_lines_count += 1
                        dist = abs(y - control_y)
                        if dist < best_dual_dist:
                            best_dual_dist, best_dual_x = dist, curr_target_x
            else:
                ...
                if not is_valid_dual_line:
                    if len(y_coords) > 0 and len(w_coords) == 0:
                        curr_target_x = int(np.mean(y_coords)) + int(w * 0.4)
                    elif len(w_coords) > 0 and len(y_coords) == 0:
                        curr_target_x = int(np.mean(w_coords)) - (int(w * 0.15) if turn_direction == "follow_right" else int(w * 0.4))
                    if curr_target_x is not None:
                        dist = abs(y - control_y)
                        if dist < best_single_dist:
                            best_single_dist, best_single_x = dist, curr_target_x

            if curr_target_x is not None:
                center_pts.append((curr_target_x, y))

        # 掃描完成：優先用雙線結果，其次單線，完全沒線才停
        if best_dual_x is not None:
            control_target_x = best_dual_x
            move_speed = self.base_speed
        elif best_single_x is not None:
            control_target_x = best_single_x
            move_speed = self.base_speed
        else:
            move_speed = 0.0
